...
# DIVERGENCE PATTERN DETECTION FUNCTION
...
def calculate_pnl(data: pd.DataFrame, initial_capital: float, trade_amount: float):
    ...
    for i in range(len(data)):
        if data.iloc[i]['Buy Signal'] == 1:
            ...
            if capital >= trade_amount:
                ...
                max_value_buy = max(max_value_buy, data.iloc[i]['Close'])

            if position > 0:
                
                if data.iloc[i]['Close'] >= 1.2 * max_value_buy:
                    sell_value = position * data.iloc[i]['Close']
                    capital += sell_value
                    position = 0
                    max_value_buy = 0

                elif data.iloc[i]['Close'] <= 0.85 * max_value_buy:
                    sell_value = position * data.iloc[i]['Close']
                    capital += sell_value
                    position = 0
                    max_value_buy = 0
            ...
            if position == 0:
                ...
                if capital >= trade_amount:
                    position -= trade_amount / data.iloc[i]['Close']
                    capital += trade_amount
                    min_value_short = min(min_value_short, data.iloc[i]['Close'])

            if position < 0:

                if data.iloc[i]['Close'] <= 0.8 * min_value_short:
                    cover_value = abs(position * data.iloc[i]['Close'])
                    capital += cover_value
                    position = 0
                    min_value_short = float('inf')

                elif data.iloc[i]['Close'] >= 1.15 * min_value_short:
                    cover_value = abs(position * data.iloc[i]['Close'])
                    capital += cover_value
                    position = 0
                    min_value_short = float('inf')
    ...
    while len(pnl) < len(data):
        pnl.append(pnl[-1]) 

    data['PnL'] = pnl[:len(data)]
    return data
